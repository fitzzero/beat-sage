...
> {
  constructor() {
    super({ model: "location", hasEntryACL: false, serviceName: "locationService" });
  }
    ...
    async (payload) => {
      ...
      const rows = await (
        this.delegate as unknown as {
          ...
          }) => Promise<Array<Pick<PrismaLocation, "id" | "name" | "difficulty">>>;
        }
      ).findMany({ skip, take: pageSize, select: { id: true, name: true, difficulty: true } });
